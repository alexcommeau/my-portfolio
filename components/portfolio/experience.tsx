...
export function Experience() {
  return (
    ...
    >
      <span id="experience" aria-hidden="true" className="pointer-events-none absolute top-24" />
      <Reveal className="mx-auto max-w-6xl px-8 py-24">
        <div className="mb-10 text-center">
          ...
          <p className="text-[15.5px] text-zinc-400">Mon parcours professionnel</p>
        </div>
        <div className="relative mx-auto max-w-3xl border-l-2 border-zinc-800 pl-8">
          {experiences.map((job) => (
            <Reveal key={job.company} variant="left" className="relative pb-10">
              ...
              <div className="mb-3 text-[14.5px] text-teal-400">{job.company}</div>
              <div className="flex max-w-[640px] flex-col gap-2">
                {job.bullets.map((bullet) => (
                  <div key={bullet} className="text-[14.5px] leading-relaxed text-zinc-400">
                    {bullet}
                  </div>
                ))}
              </div>
